...
ipcMain.on("generate-captions", () => {
  ...
  console.log("Starting caption generation for:", currentFilePath);

  mainWindow.webContents.send("caption-start");

  const py = spawn("python", ["-u", "generate_caption.py", currentFilePath]);
  ...
  py.stdout.on("data", (data) => {
    ...
    if (msg.startsWith("PARTIAL:")) {
      ...
      // update your subtitle overlay in Electron
  ...
  py.on("close", () => {
    console.log("Captions process finished.");
  });
});