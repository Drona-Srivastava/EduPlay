...
class SubtitlesManager {
  ...
  adjustTimestamp(timestamp, delayInSeconds) {
    ...
    return `${newHours.toString().padStart(2, "0")}:${newMinutes
      .toString()
      .padStart(2, "0")}:${newSeconds.toString().padStart(2, "0")}.${newMs
      .toString()
      .padStart(3, "0")}`;
  }
  ...
  toggleDebugOverlay() {
    ...
    const updateDebugInfo = () => {
      ...
      overlay.innerHTML = `
        ...
        ${
          activeTrack
            ? `<div>Track Path: ${activeTrack.dataset.originalPath.substring(
                0,
                30
              )}...</div>`
            : ""
        ...
        ${
          activeTrack
            ? `<div>Track Src: ${activeTrack.src.substring(0, 30)}...</div>`
            : ""
        }
  ...
  initializeSubtitleMenu() {
      ...
      <div class="subtitle-options">
        ...
        <div class="subtitle-delay-controls">
          <div class="subtitle-delay-header">
            ...
            <span id="subtitle-delay-display">${this.subtitleDelay.toFixed(
              1
            )}s</span>
          </div>
        ...
        <div class="subtitle-controls">
          ...
          <button id="generate-captions">Generate Captions</button>
          <label>
            <input type="checkbox" id="auto-load-subtitles" ${
              this.autoLoadEnabled ? "checked" : ""
            }>
            Auto-load subtitles
          </label>
      </div>
    ...
    document
      .getElementById("generate-captions")
      .addEventListener("click", () => {
        // Tell main process to start generating captions
        ipcRenderer.send("generate-captions");
        subtitleMenu.style.display = "none";
        // Also tell renderer to start loader + pause video
        ipcRenderer.send("caption-start");
      });

    ipcRenderer.on("caption-done", async (event, { filePath }) => {
      console.log("Received caption file in subtitle.js:", filePath);

      try {
        await this.addSubtitleTrack(filePath); // 1️⃣ add track first
        console.log("Subtitle track added successfully!");

        await this.setActiveSubtitle(filePath); // 2️⃣ then activate it
        console.log("Subtitle activated successfully!");
      } catch (err) {
        console.error("Failed to add/activate subtitle:", err);
      }
    });
  ...
  async searchInActiveSubtitles(query) {
    try {
      if (!this.lastSuccessfulSubtitle) {
        console.warn("No active subtitle file");
        return [];
      }

      // Read the active subtitle file
      const content = await fs.readFile(this.lastSuccessfulSubtitle, "utf-8");
      ...
      for (const block of blocks) {
        ...
        if (lines.length >= 3) {
          ...
          if (textLines.toLowerCase().includes(query.toLowerCase())) {
            const startTime = timeLine.split("-->")[0].trim();
            results.push(this.timestampToSeconds(startTime));
          }
        }
      }
  ...
  timestampToSeconds(timestamp) {
    const [h, m, s] = timestamp.replace(",", ".").split(":");
            ...
            <div class="subtitle-section">
                ${Array.from(this.mediaPlayer.getElementsByTagName("track"))
                  ...
                  .map(
                    (track) => `
                        <div class="subtitle-item ${
                          track === activeTrackElement ? "active" : ""
                        }"
                             data-path="${track.dataset.originalPath.replace(
                               /"/g,
                               "&quot;"
                             )}"
                             data-embedded="true"
